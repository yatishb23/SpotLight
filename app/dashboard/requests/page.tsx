    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 border-b border-neutral-900 pb-10">
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm text-neutral-500 font-light">
              Reviewing pending organizer identities for registry access.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <Button
            variant="outline"
            className="bg-transparent border-neutral-800 hover:bg-neutral-900 text-neutral-400"
          >
      ...
      <div className="flex flex-col md:flex-row gap-4 justify-between items-center">
        <div className="flex items-center gap-2 text-neutral-500">
          ...
          <span className="text-[10px] font-mono uppercase tracking-widest">
            Status: Monitoring Pending Nodes
          </span>
        </div>
          ...
          <Table>
            <TableHeader className="bg-neutral-900/50">
              <TableRow className="border-neutral-800 hover:bg-transparent">
                <TableHead className="text-[10px] uppercase tracking-widest font-bold text-neutral-500">
                  Identity Protocol
                </TableHead>
                <TableHead className="text-[10px] uppercase tracking-widest font-bold text-neutral-500">
                  Node Credentials
                </TableHead>
                <TableHead className="text-right text-[10px] uppercase tracking-widest font-bold text-neutral-500">
                  Registry Action
                </TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filteredRequests.map((req) => (
                <TableRow
                  key={req.id}
                  className="border-neutral-900 hover:bg-neutral-900/30 transition-colors"
                >
                  ...
                  <TableCell>
                    <div className="flex flex-col gap-1.5">
                      <div className="flex gap-2">
                        <Badge
                          variant="outline"
                          className="text-[9px] uppercase font-bold border-neutral-800 text-blue-400 bg-blue-400/5"
                        >
                          {req.role}
                        </Badge>
                        <Badge
                          variant="outline"
                          className="text-[9px] uppercase font-bold border-neutral-800 text-neutral-500"
                        >
                          Pending_Auth
                        </Badge>
                      </div>
                      <span className="text-xs font-mono text-neutral-500 lowercase">
                        {req.email}
                      </span>
                    </div>
                  </TableCell>
          ...
          {filteredRequests.length === 0 && (
            <div className="py-32 text-center flex flex-col items-center justify-center space-y-4 opacity-20">
              ...
              <p className="text-[10px] font-bold uppercase tracking-[0.4em]">
                Registry Clear // No Pending Nodes
              </p>
            </div>
          )}
        </CardContent>
      ...
      <div className="pt-10 flex flex-col md:flex-row justify-between items-center opacity-30 border-t border-neutral-900 gap-4">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[9px] font-mono uppercase tracking-[0.2em]">
            Secure Connection: AES-256-GCM Active
          </span>
        </div>
        <p className="text-[9px] font-mono uppercase tracking-[0.2em]">
          Node Registry Last Sync: {new Date().toLocaleTimeString()}
        </p>
      </div>
    </div>
  );
